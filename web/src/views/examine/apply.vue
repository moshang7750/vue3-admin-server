<template>
  ...
  <div class="af-filter-warp">
    <div class="af-warp-left">
      <el-select v-model="params.applyType" placeholder="请选择申请类型">
        <el-option
      ...
      <el-input v-model="params.applyName" placeholder="请输入申请姓名" />
    </div>
  ...
  <el-table :data="tableData" style="width: 100%">
    <el-table-column prop="id" label="申请ID" sortable> </el-table-column>
    <el-table-column prop="applyTypeName" label="申请类型" sortable>
    </el-table-column>
    <el-table-column prop="affairName" label="事务名" sortable>
    </el-table-column>
...
import { ApplyCompontFilter, ApplyCompontTable } from '@/types/examine.types'
import { useMethons } from '@/libs/useMethons'
export default defineComponent({
  setup(props) {
    // 筛选框
    const params: ApplyCompontFilter = reactive({
      applyType: '',
      applyName: '',
      page: 1,
      size: 10,
    ...
    const tableData: Ref<ApplyCompontTable[]> = ref([
      {
        id: '111',
        applyType: 1,
        applyTypeName: '账号升级申请',
        applyName: '测试',
        affairName: '免费下载',
        examiner: '李四',
